            ...
            For i = 0 To SpellDBC.Rows - 1
                Try
                    ...
                    If Int(SpellDBC.Item(i, 71)) <> 0 Then
                        ...
                        SPELLs(ID).SpellEffects(0).diceBase = SpellDBC.Item(i, 77)
                        SPELLs(ID).SpellEffects(0).dicePerLevel = SpellDBC.Item(i, 80)
                        SPELLs(ID).SpellEffects(0).valuePerLevel = SpellDBC.Item(i, 83, DBC.DBCValueType.DBC_FLOAT)
                        SPELLs(ID).SpellEffects(0).valueDie = SpellDBC.Item(i, 74)
                        SPELLs(ID).SpellEffects(0).valueBase = SpellDBC.Item(i, 86)
                        SPELLs(ID).SpellEffects(0).Mechanic = SpellDBC.Item(i, 89)
                        SPELLs(ID).SpellEffects(0).implicitTargetA = SpellDBC.Item(i, 92)
                        SPELLs(ID).SpellEffects(0).implicitTargetB = SpellDBC.Item(i, 95)
                        SPELLs(ID).SpellEffects(0).RadiusIndex = SpellDBC.Item(i, 98)
                        SPELLs(ID).SpellEffects(0).ApplyAuraIndex = SpellDBC.Item(i, 101)
                        SPELLs(ID).SpellEffects(0).Amplitude = SpellDBC.Item(i, 104)
                        'MultipleValue = 107
                        SPELLs(ID).SpellEffects(0).ChainTarget = SpellDBC.Item(i, 110)
                        SPELLs(ID).SpellEffects(0).ItemType = SpellDBC.Item(i, 113)
                        SPELLs(ID).SpellEffects(0).MiscValue = SpellDBC.Item(i, 116)
                        SPELLs(ID).SpellEffects(0).MiscValueB = SpellDBC.Item(i, 119)
                        SPELLs(ID).SpellEffects(0).TriggerSpell = SpellDBC.Item(i, 122)
                        SPELLs(ID).SpellEffects(0).valuePerComboPoint = SpellDBC.Item(i, 125)
                    Else
                    ...
                    If Int(SpellDBC.Item(i, 72)) <> 0 Then
                        ...
                        SPELLs(ID).SpellEffects(1).diceBase = SpellDBC.Item(i, 78)
                        SPELLs(ID).SpellEffects(1).dicePerLevel = SpellDBC.Item(i, 81)
                        SPELLs(ID).SpellEffects(1).valuePerLevel = SpellDBC.Item(i, 84, DBC.DBCValueType.DBC_FLOAT)
                        SPELLs(ID).SpellEffects(1).valueDie = SpellDBC.Item(i, 75)
                        SPELLs(ID).SpellEffects(1).valueBase = SpellDBC.Item(i, 87)
                        SPELLs(ID).SpellEffects(1).Mechanic = SpellDBC.Item(i, 90)
                        SPELLs(ID).SpellEffects(1).implicitTargetA = SpellDBC.Item(i, 93)
                        SPELLs(ID).SpellEffects(1).implicitTargetB = SpellDBC.Item(i, 96)
                        SPELLs(ID).SpellEffects(1).RadiusIndex = SpellDBC.Item(i, 99)
                        SPELLs(ID).SpellEffects(1).ApplyAuraIndex = SpellDBC.Item(i, 102)
                        SPELLs(ID).SpellEffects(1).Amplitude = SpellDBC.Item(i, 105)
                        'MultipleValue = 108
                        SPELLs(ID).SpellEffects(1).ChainTarget = SpellDBC.Item(i, 111)
                        SPELLs(ID).SpellEffects(1).ItemType = SpellDBC.Item(i, 114)
                        SPELLs(ID).SpellEffects(1).MiscValue = SpellDBC.Item(i, 117)
                        SPELLs(ID).SpellEffects(1).MiscValueB = SpellDBC.Item(i, 120)
                        SPELLs(ID).SpellEffects(1).TriggerSpell = SpellDBC.Item(i, 123)
                        SPELLs(ID).SpellEffects(1).valuePerComboPoint = SpellDBC.Item(i, 126)
                    Else
                    ...
                    If Int(SpellDBC.Item(i, 73)) <> 0 Then
                        ...
                        'SPELLs(ID).SpellEffects(0).diceSides = SpellDBC.Item(i, 76)
                        SPELLs(ID).SpellEffects(2).diceBase = SpellDBC.Item(i, 79)
                        SPELLs(ID).SpellEffects(2).dicePerLevel = SpellDBC.Item(i, 82)
                        SPELLs(ID).SpellEffects(2).valuePerLevel = SpellDBC.Item(i, 85, DBC.DBCValueType.DBC_FLOAT)
                        SPELLs(ID).SpellEffects(2).valueDie = SpellDBC.Item(i, 76)
                        SPELLs(ID).SpellEffects(2).valueBase = SpellDBC.Item(i, 88)
                        SPELLs(ID).SpellEffects(2).Mechanic = SpellDBC.Item(i, 91)
                        SPELLs(ID).SpellEffects(2).implicitTargetA = SpellDBC.Item(i, 94)
                        SPELLs(ID).SpellEffects(2).implicitTargetB = SpellDBC.Item(i, 97)
                        SPELLs(ID).SpellEffects(2).RadiusIndex = SpellDBC.Item(i, 100)
                        SPELLs(ID).SpellEffects(2).ApplyAuraIndex = SpellDBC.Item(i, 103)
                        SPELLs(ID).SpellEffects(2).Amplitude = SpellDBC.Item(i, 106)
                        'MultipleValue = 109
                        SPELLs(ID).SpellEffects(2).ChainTarget = SpellDBC.Item(i, 112)
                        SPELLs(ID).SpellEffects(2).ItemType = SpellDBC.Item(i, 115)
                        SPELLs(ID).SpellEffects(2).MiscValue = SpellDBC.Item(i, 118)
                        SPELLs(ID).SpellEffects(2).MiscValueB = SpellDBC.Item(i, 121)
                        SPELLs(ID).SpellEffects(2).TriggerSpell = SpellDBC.Item(i, 124)
                        SPELLs(ID).SpellEffects(2).valuePerComboPoint = SpellDBC.Item(i, 127)
                    Else
                        SPELLs(ID).SpellEffects(2) = Nothing
                    End If

                    'EffectSpellClassMaskA1 = 128
                    'EffectSpellClassMaskA2 = 129
                    'EffectSpellClassMaskA3 = 130
                    'EffectSpellClassMaskB1 = 131
                    'EffectSpellClassMaskB2 = 132
                    'EffectSpellClassMaskB3 = 133
                    'EffectSpellClassMaskC1 = 134
                    'EffectSpellClassMaskC2 = 135
                    'EffectSpellClassMaskC3 = 136
                    SPELLs(ID).SpellVisual = SpellDBC.Item(i, 137)
                    'SpellVisual2 = 138
                    'SpellIconID = 139
                    'ActiveIconID = 140
                    'SpellPriority = 141
                    'SpellName (string*16) = 142-157
                    'SpellNameFlag = 158
                    'Rank (string*16) = 159-174
                    'RankFlags = 175
                    'Description (string*16) = 176-191
                    'DescriptionFlags = 192
                    'Tooltip (string*16) = 193-208
                    'TooltipFlags = 209
                    SPELLs(ID).manaCostPercent = SpellDBC.Item(i, 210)
                    'StartRecoveryCategory = 211
                    'StartRecoveryTime = 212
                    'MaxTargetLevel = 213
                    'SpellFamilyName = 214
                    'SpellFamilyFlags0 = 215 \
                    'SpellFamilyFlags1 = 216 |-12 bytes flag mask
                    'SpellFamilyFlags2 = 217 /
                    SPELLs(ID).MaxTargets = SpellDBC.Item(i, 218)
                    SPELLs(ID).DamageType = SpellDBC.Item(i, 219)
                    'PreventionType = 220
                    'StanceBarOrder = 221
                    SPELLs(ID).DamageMultiplier = SpellDBC.Item(i, 222, DBCValueType.DBC_FLOAT)
                    'DmgMultiplier2 = 223
                    'DmgMultiplier3 = 224
                    'MinFactionId = 225
                    'MinReputation = 226
                    'RequiredAuraVision = 227
                    SPELLs(ID).TotemCategory(0) = SpellDBC.Item(i, 228)
                    SPELLs(ID).TotemCategory(1) = SpellDBC.Item(i, 229)
                    SPELLs(ID).RequiredAreaID = SpellDBC.Item(i, 230)
                    SPELLs(ID).School = SpellDBC.Item(i, 231)
                    'runeCostID = 232
                    'spellMissileID = 233
                    'PowerDisplayId = 234               ' Integer - PowerDisplay.dbc, new in 3.1
                    'unk_320_4[3] = 235-237             ' Float - 3.2.0
                    'spellDescriptionVariableID = 238   ' Integer - 3.2.0
                    'SpellDifficultyId = 239            ' Integer - 3.3.0
