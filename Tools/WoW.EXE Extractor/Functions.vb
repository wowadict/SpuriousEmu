    ...
    Public Sub ExtractUpdateFields()
        ...
        Else
            ...
            For j As Integer = 0 To Info.Count - 1
                    ...
                    If LastFieldType <> sField Then
                        If LastFieldType <> "" Then
                            ...
                            If LastFieldType.ToLower = "object" Then
                                w.WriteLine("    {0,-78}", LastFieldType.ToUpper & "_END = &H" & Hex(Info(j - 1).Offset + 1))
                            Else
                                w.WriteLine("    {0,-78}' 0x{1:X3}", LastFieldType.ToUpper & "_END = " & BasedOnName & " + &H" & Hex(Info(j - 1).Offset + 1), BasedOn + Info(j - 1).Offset + 1)
                            End If
                ...
                End If
            Next j
            If LastFieldType <> "" Then w.WriteLine("    {0,-78}' 0x{1:X3}", LastFieldType.ToUpper & "_END = " & BasedOnName & " + &H" & Hex(Info(Info.Count - 1).Offset + 1), BasedOn + Info(Info.Count - 1).Offset + 1)
            w.WriteLine("End Enum")
            w.Flush()
