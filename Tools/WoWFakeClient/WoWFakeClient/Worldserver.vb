...
Public Module Worldserver
    ...
    Sub InitializePackets()
        ...
        PacketHandlers(OPCODES.SMSG_TIME_SYNC_REQ) = CType(AddressOf On_SMSG_TIME_SYNC_REQ, HandlePacket)
    End Sub
    ...
    Sub OnConnect()
        ...
        PingTimer = New Timer(AddressOf Ping, Nothing, 0, 20000)
    End Sub
    ...
    Sub Ping(ByVal State As Object)
        Try
            ...
            Console.WriteLine("[{0}][World] Ping.", Format(TimeOfDay, "HH:mm:ss"))
        Catch e As Exception
            PingTimer.Dispose()
        End Try
    End Sub
    ...
    Sub OnData()
        While Queue.Count > 0
            ...
            If PacketHandlers.ContainsKey(Packet.OpCode) = True Then
                ...
                Catch e As Exception
                    Console.WriteLine("Opcode handler {2}:{2:X} caused an error:{1}{0}", e.Message, vbNewLine, Packet.OpCode)
                End Try
            Else
    ...
    Sub Encode(ByRef Buffer() As Byte)
        ...
        For T = 0 To 5
            Buffer(T) = CByte((((bSS_Hash(Key(3)) Xor Buffer(T)) And &HFF) + Key(2)) And &HFF)
            Key(2) = Buffer(T)
            Key(3) = CByte((Key(3) + 1) Mod 20)
        Next T
    End Sub
